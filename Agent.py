...
from QNetwork_dueling import QNetwork_dueling
...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, double_dqn=False, dueling=False, prioritized_replay=False):
    ...
    def learn(self, experiences, gamma):
        ...
        if self.double_dqn:
            argmax_action = self.qnetwork_local(next_states).argmax(dim=1).unsqueeze(-1)
            Q_targets_next = self.qnetwork_target(next_states).gather(1, argmax_action)
